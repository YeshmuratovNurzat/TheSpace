...
import com.etoolkit.thespace.databinding.ItemLaunchPastBinding
import com.etoolkit.domain.launches.model.Launch

class LaunchesPastAdapter : RecyclerView.Adapter<LaunchesPastAdapter.LaunchesHolder>() {

    private var launches = emptyList<Launch>()

    var onClick : ((Launch) -> Unit)? = null

    class LaunchesHolder(private val binding: ItemLaunchPastBinding) : RecyclerView.ViewHolder(binding.root){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LaunchesHolder {
        val binding = ItemLaunchPastBinding.inflate(LayoutInflater.from(parent.context),parent,false)
